...
<template>
    <div class="basis-full bg-white rounded-2xl justify-between flex flex-col">
        ...
        <div class="bg-gray-100 px-2 m-4 rounded-xl flex flex-row items-center basis-1/12">
            ...
            <i class="fa fa-paperclip pr-3 cursor-pointer"></i>
            <textarea class="bg-gray-100 w-full p-3 outline-none resize-none" rows="1" onkeydown="handleKeyDown(event)"
                placeholder="Type a message..." id="message"></textarea>
            <i class="fa-regular fa-paper-plane px-3 cursor-pointer" onclick="sendMessage()"></i>
        </div>
    </div>
</template>